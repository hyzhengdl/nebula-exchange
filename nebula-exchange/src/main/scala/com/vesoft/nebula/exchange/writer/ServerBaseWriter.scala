...
abstract class ServerBaseWriter extends Writer {
  ...
  private[this] val EDGE_ID_TEMPLATE                    = "%s->%s@%d"
  private[this] val EDGE_ID_WITHOUT_RANKING_TEMPLATE    = "%s->%s"

  def toDeleteSentence(vertices: Vertices): String = {
    BATCH_DELETE_VERTEX_TEMPLATE.format(
      Type.VERTEX.toString,
      vertices.values
        .map { vertex => vertex.vertexID}.mkString(", ")
    )
  }
  ...
  def toDeleteSentence(name: String, edges: Edges): String = {
    val values = edges.values
      .map { edge =>
        (for (element <- edge.source.split(","))
          yield {
            if (edge.ranking.isEmpty)
              EDGE_ID_WITHOUT_RANKING_TEMPLATE.format(element, edge.destination)
            else
              EDGE_ID_TEMPLATE.format(element, edge.destination, edge.ranking.get)
          }
          ).mkString(", ")
      }.mkString(", ")
  ...
  def deleteVertices(vertices: Vertices): String

  def writeEdges(edges: Edges): String

  def writeNgql(ngql: String): String

  def deleteEdges(edges: Edges): String
...
class NebulaGraphClientWriter(dataBaseConfigEntry: DataBaseConfigEntry,
  ...
  override def deleteVertices(vertices: Vertices): String = {
    val sentence = toDeleteSentence(vertices)
    LOG.info(s"sentence is ${sentence}")
    if (rateLimiter.tryAcquire(rateConfig.timeout, TimeUnit.MILLISECONDS)) {
      val result = graphProvider.submit(session, sentence)
      if (result.isSucceeded) {
        return null
      }
      LOG.error(s"delete vertex failed for ${result.getErrorMessage}")
    } else {
      LOG.error(s"delete vertex failed because write speed is too fast")
    }
    LOG.info(sentence)
    sentence
  }
  ...
  override def deleteEdges(edges: Edges): String = {
    val sentence = toDeleteSentence(config.name, edges)
    if (rateLimiter.tryAcquire(rateConfig.timeout, TimeUnit.MILLISECONDS)) {
      val result = graphProvider.submit(session, sentence)
      if (result.isSucceeded) {
        return null
      }
      LOG.error(s"delete edge failed for ${result.getErrorMessage}")
    } else {
      LOG.error(s"write vertex failed because write speed is too fast")
    }
    LOG.info(sentence)
